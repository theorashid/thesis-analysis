    ...
    for (y2 in c(2010, 2019)) {
      if (y1 != y2) {
        ...
        p_hist <- e0_decompose |>
          ...
          labs(x = "", y = "Average contribution across districts (years)") +
          theme_thesis() +
          theme(
...
```{r decomposition change 2002-10 2010-19}
for (s in sexes) {
  if (s == "female") id <- 2 else id <- 1
  e0_decompose_200210 <- read_rds(
    here(
      "data",
      "life_table",
      str_c(
    ...
    as_tibble() |>
    rename(Dx = Freq) |>
    mutate(period = "2002-10") |>
    group_by(LAD) |>
    mutate(de0 = sum(Dx)) |>
    ungroup() |>
    group_by(cause) |>
    mutate(rank_de0 = rank(de0))

  e0_decompose_201019 <- read_rds(
    here(
      "data",
      "life_table",
      str_c(
  ...
  p <- e0_decompose_200210 |>
    bind_rows(e0_decompose_201019) |>
    ggplot(aes(x = rank_de0, y = Dx, colour = period)) +
    # geom_point(size = 0.1) +
    geom_smooth(linewidth = 0.2, se = FALSE) +
