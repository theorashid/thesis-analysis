...
```{r correlation plot data}
correlations <- list()
for (s in sexes) {
  correlation <- list()
  for (decile in 1:10) {
    correlation[[decile]] <- cor(
      london_change |>
        filter(sex == s) |>
        filter(decile_house_price_2002 == decile) |>
        select(-c(sex, decile_house_price_2002)),
      method = "spearman"
    )["de0", ]
  }
  ...
  correlation <- bind_rows(correlation) |>
    select(-de0) |>
    rename(
      `House price` = d_med_OA_log_house_price,
      `Poverty` = d_income_rank,
      `Unemployment` = d_employment_rank,
      `Education deprivation` = d_education_rank,
      `Population turnover` = churn,
      `Share of population, 0-14` = d_pop_0_14_rel,
      `Share of population, 15-29` = d_pop_15_29_rel,
      `Share of population, 30-69` = d_pop_30_69_rel,
      `Share of population, 70+` = d_pop_70_plus_rel
    ) |>
    rowid_to_column(var = "Decile")

  correlation <- correlation |>
    pivot_longer(cols = -one_of("Decile")) |>
    mutate(
      name_factor = factor(
        name,
        levels = c(
          ...
          "Unemployment",
          "Poverty",
          "Share of population, 70+",
          "Share of population, 30-69",
          "Share of population, 15-29",
          "Share of population, 0-14",
          "Population turnover",
          "House price"
        )
      ),
      decile_factor = factor(
        str_c("D", Decile),
        levels = str_c("D", 1:10)
      )
    )
...
for (s in sexes) {
  correlation_plots[[s]] <- ggplot(
    ...
    labs(
      x = "",
      y = "Change in variable, 2002-2019",
      caption = s
    ) +
    theme_thesis() +
    pair_skeleton
...
legend <- get_legend(correlation_plots[["women"]])
grid <- plot_grid(
  correlation_plots[["women"]] + theme(legend.position = "none"),
  correlation_plots[["men"]] +
    theme(
...
grid <- plot_grid(legend, grid, ncol = 1, rel_heights = c(.1, 1))
