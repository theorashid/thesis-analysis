...
```{r data}
args <- lst(region = "LAD", sex = "male", model = "car_as_at")
...
```{r d80q0 correlation plot in 2019}
p <- median_q_change |>
  pivot_wider(
    id_cols = c(LAD),
    names_from = cause,
...
save_plot(
  here(
    ...
    "cancers",
    "figures",
    str_c(args$sex, "_correlation_change", ".pdf")
  ),
  p
)
```
...
for (disease in cause_list) {
  ...
  save_plot(
    here(
      ...
      "cancers",
      "figures",
      str_c(args$sex, "_series_", str_replace_all(disease, " ", "_"), ".pdf")
    ),
    p
  )
}
...
for (disease in cause_list) {
  ...
  save_plot(
    here(
      ...
      "cancers",
      "figures",
      str_c(args$sex, "_IMD_", str_replace_all(disease, " ", "_"), ".pdf")
    ),
    p
  )
}
