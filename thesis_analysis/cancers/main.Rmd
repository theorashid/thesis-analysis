...
quantile_q <- apply(
  ...
  left_join(
    lookup |>
      mutate(GOR2011NM = factor(GOR2011NM, levels = regions)) |>
      group_by(GOR2011, GOR2011NM) |>
      summarise(n_lads = n()) |>
      ungroup() |>
      arrange(GOR2011NM) |>
      mutate(csum_lads = cumsum(n_lads) - n_lads) |>
      select(-GOR2011NM)
  ) |>
  arrange() |>
  pivot_wider(names_from = quantile, values_from = `80q0`) |>
  group_by(year, cause, GOR2011) |>
  mutate(rank = rank(`50%`)) |>
  ungroup() |>
  mutate(rank = rank + csum_lads)
```
...
save_plot(
  here(
    ...
    "cancers",
    "figures",
    str_c(args$sex, "_distribution1", ".pdf")
  ),
  p
)
```
