...
median_q_rel_change <- apply(
  X = (cause_q["2019", , , ] - cause_q["2002", , , ]) / cause_q["2002", , , ],
  MARGIN = c(1, 3),
  FUN = median
) |>
  as.data.frame.table(stringsAsFactors = FALSE) |>
...
apply(
  X = (cause_q["2019", , , ] - cause_q["2002", , , ]) * 100 / cause_q["2002", , , ],
  MARGIN = c(1, 3),
  FUN = quantile,
  p = c(0.025, 0.5, 0.975)
) |>
...
for (disease in cause_list) {
  ...
  map_change <- shape |>
    left_join(
      median_q_rel_change |> filter(cause == disease) |> mutate(LAD2020 = LAD)
    ) |>
...
p <- median_q |>
  ...
  labs(caption = "Probability of dying between birth and 80 in 2019") +
  theme_thesis() +
  theme(
...
p_change <- median_q_rel_change |>
  ...
  labs(caption = "Change in probability of death from 2002 to 2019") +
  theme_thesis() +
  theme(
    axis.line = element_blank(),
