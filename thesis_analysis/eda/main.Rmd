...
p <- mx |>
  ...
  geom_line() +
  scale_x_continuous(expand = c(0, NA), breaks = breaks_pretty(10)) +
...
p <- mx |>
  mutate(
    broad_age_group = case_when(
      age_group < 20 ~ "0-19 years",
      age_group >= 20 & age_group < 60 ~ "20-59 years",
      age_group >= 60 ~ "80+ years",
    )
  ) |>
  ...
  geom_line() +
  facet_wrap(~broad_age_group, scales = "free_y") +
  # scale_x_continuous(expand = c(0, NA), breaks = breaks_pretty(10)) +
  # scale_y_continuous(trans = "log10") +
  theme_thesis() +
