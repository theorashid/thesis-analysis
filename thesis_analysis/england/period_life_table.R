...
.KTExtension <- function(lx70) {
  ...
  ax70 <- as.vector(t(sapply(
    split(seq(nrow(dx70)), x5y),
    function(v) {
      colSums(dx70[v, , drop = FALSE] * yl[v]) /
...
PeriodLifeTable <- function(age, mx, ax, sex, check.conv = FALSE) {
  ...
  # return e0
  e0 <- ex[seq.int(1, length(ex), length(unique(age)))]
  return(e0)
}
